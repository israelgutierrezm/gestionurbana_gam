...
$pdf->SetXY(80, 22);
$pdf->Cell(100, 10, 'ACTA DE CALIFICACIONES RECURSAMIENTO', 0, 1, 'C', 0);
...
$query_alumnos = query('SELECT distinct tm.alumno_id, p.nombre, p.primer_apellido, p.segundo_apellido,
tac.actividad_nombre,tmfa.calificacion
FROM '.$GLOBALS['db_controlEscolar'].'.inter_asignatura_grupo iag
join '.$GLOBALS['db_controlEscolar'].'.inter_alumno_asignatura_grupo ida on ida.asignatura_grupo_id = iag.asignatura_grupo_id
join '.$GLOBALS['db_controlEscolar'].'.tr_alumno ta on ta.alumno_id = ida.alumno_id
...
join '.$GLOBALS['db_learning'].'.inter_modulos_asignatura_grupo imag on imag.asignatura_grupo_id = ida.asignatura_grupo_id
join '.$GLOBALS['db_learning'].'.tr_actividad tac on tac.modulo_id = imag.modulo_id
left join '.$GLOBALS['db_learning'].'.tr_materia_fecha_actividad tmfa on tmfa.materia_id = tm.materia_id and tmfa.actividad_id = tac.actividad_id
where tm.asignatura_grupo_id= '.$id_asignatura_grupo.' and tac.actividad_id  = '.$id_actividad.' and ta.estatus=1 and
iag.estatus=1 and ida.estatus =1 and tm.estatus=1 and imag.estatus =1 and tac.estatus=1 and tmfa.estatus=1');
