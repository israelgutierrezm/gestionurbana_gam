...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        // $this->Image($GLOBALS['url_front'].'/assets/images/logo.png', 10, 10, 65, 25);
    }
...
AND tm.estatus=1 and p.estatus = 1 and tm.tipo_materia_id = 4 and tal.estatus = 1 order by p.primer_apellido, p.segundo_apellido, p.nombre');
...
while ($arreglo_alumno = arreglo($query_alumnos)) {
    ...
    $pdf->Cell($ancho_columna_2, 6, utf8_decode(mb_strtoupper($arreglo_alumno['nombre'] . ' ' . $arreglo_alumno['primer_apellido'] . ' ' . $arreglo_alumno['segundo_apellido'])), 0, 0, 'L');
