   ...
   $fila_alumno =5;
    ...
    while($arreglo =  arreglo($query)){
      ...
      $objPHPExcel->getActiveSheet()->setCellValue('D'.$fila_valores,'Egresado');            
          ...
          join tr_grupo tg on tg.plan_orden_id = ipo.plan_orden_id  
          join cat_ciclo cc on cc.ciclo_id = tg.ciclo_id 
          where ta.estatus = 1 and tg.estatus=1 and cc.estatus=1 and tg.ciclo_id ='.$id_ciclo.' and tpe.carrera_id ='.$arreglo['carrera_id'].' '.$script2.' group by ta.situacion_alumno_id) t1
...
if($GLOBALS['version']==5){
  ...
  // include '../../extras/excel/crear7.php';  
   ...
   header('Content-Disposition: attachment;filename="AlumnosOfertaCiclo.xls"');
